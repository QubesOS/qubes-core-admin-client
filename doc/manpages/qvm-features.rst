...
enough of them, new ones will be created, if there are more than enough, the
excess will be removed.

There are some cases where a new disposable won't be preloaded:

-  When every preloaded disposable is killed before being used.

-  When there is not enough available memory to preload one disposable and:

   -  this feature has just been set.

   -  the last remaining preloaded disposable has been used.

It is possible to resume from this state by setting this feature again to the
same value it already has and it will try to preload the difference (if there is
enough memory to preload at least one disposable).
...
.. warning::
   ...
   template itself will be shown before the preloaded disposable can be paused.
...
interrupted for pause or suspend, therefore it is allowed to skip them.
