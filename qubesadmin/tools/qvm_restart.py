...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="kill selected domains instead of shutting them down."
)
parser.add_argument(
    "--start","-s",
    action="store_true",
...
def report_failure(data):
    ...
    """
    nwidth = str(max(len(vm.name) for vm in data))
    output = [
        ("{:<" + nwidth + "} REASON\n").format("NAME")
    ]
    for vm in data:
...
async def run_async(args=None, app=None):
    """Main routine of :program:`qvm-restart`. See `qvm-restart --help` or 
    execute this function with args=["--help"] for more information.

    Requires the following admin API services to function:

    admin.vm.CurrentState
    admin.vm.Start
    admin.vm.Shutdown+wait
    admin.vm.property.Get+auto_cleanup
    """
    ...
    restart = qubesadmin.utils.restart(
        target_domains, force=args.force, kill_domains=args.kill
    )

    failed_shutdown = await restart.__anext__()
    if failed_shutdown:
        parser.print_error(
            "Failed to restart domains:\n",
            *report_failure(failed_shutdown),
            end=""
        )
    failed_start = await restart.__anext__()
    if failed_start:
        parser.print_error(
    ...
    if failed_start or failed_shutdown:
        return 75

def main(args=None, app=None):
    """
    Command line entry point of :program:`qvm-restart`. Only purpose is to 
