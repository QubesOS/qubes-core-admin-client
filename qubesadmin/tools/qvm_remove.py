...
def main(args=None, app=None):  # pylint: disable=missing-docstring
    ...
    if args.all_domains and not (args.no_confirm or args.exclude):
        print(
            ...
            "unrecoverable state!", file=sys.stderr
        )
    ...
    if not args.no_confirm:
        print("This will completely remove the selected VM(s)...", file=sys.stderr)
        ...
        for vm in args.domains:
            try:
                ...
                preloads = [
                    ...
                    if holder and getattr(holder, "is_preload", False)
                    and (
                        prop == "template"
                        or (
                            prop == "default_dispvm"
                ...
                if dependencies:
                    print(
                        "VM {} cannot be removed. It is in use as:".format(vm.name), file=sys.stderr
                    )
                    for holder, prop in dependencies:
                        if holder:
                            print(" - {} for {}".format(prop, holder.name), file=sys.stderr)
                        else:
                            print(" - global property {}".format(prop), file=sys.stderr)
