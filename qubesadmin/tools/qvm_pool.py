...
def set_pool(args):
    ...
    for opt, value in options:
        ...
        try:
            ...
            errors.append(
                "Failed to set option %s for pool %s: %s\n"
                % (opt, pool.name, str(e))
            )
    if errors:
        raise qubesadmin.exc.QubesException("\n".join(errors))
...
def init_info_parser(sub_parsers):
    ...
    i_parser.add_argument(
        metavar="POOL_NAME", dest="pools", action=qubesadmin.tools.PoolsAction
    )
...
def uses_legacy_options(args, app):
    ...
    parser = argparse.ArgumentParser(
        description=__doc__, usage=argparse.SUPPRESS, add_help=False
    )

    parser.add_argument(
        "-a",
        "--add",
        dest="has_legacy_options",
        action="store_true",
