...
def get_parser() -> argparse.ArgumentParser:
    ...
    def parser_add_command(cmd, help_str):
        return subparsers.add_parser(
            cmd, formatter_class=formatter, help=help_str, description=help_str
        )
    ...
    parser_main.add_argument(
        ...
        help=(
            "Specify files containing DNF repository configuration."
    ...
    parser_main.add_argument(
        ...
        default=UPDATEVM,
        help=(
            "Specify VM to download updates from."
    ...
    parser_main.add_argument(
        ...
        dest="repos",
        help=(
            "Disable certain repositories by an id or a glob."
            " Can be used more than once."
        ),
    )
    parser_main.add_argument(
    ...
    parser_main.add_argument(
        "--cachedir", default=CACHE_DIR, help="Specify cache directory."
    )
    parser_main.add_argument(
        "--keep-cache",
        action="store_true",
    ...
    parser_main.add_argument(
        "--yes", action="store_true", help='Assume "yes" to questions.'
    )
    # qvm-template {install,reinstall,downgrade,upgrade}
    parser_install = parser_add_command(
        "install", help_str="Install template packages."
    )
    parser_install.add_argument(
        "--pool", help="Specify storage pool to store created templates in."
    )
    parser_reinstall = parser_add_command(
        "reinstall", help_str="Reinstall template packages."
    )
    parser_downgrade = parser_add_command(
        "downgrade", help_str="Downgrade template packages."
    )
    parser_upgrade = parser_add_command(
        "upgrade", help_str="Upgrade template packages."
    )
    for parser_x in [
        parser_install,
        parser_reinstall,
    ...
    parser_download = parser_add_command(
        "download", help_str="Download template packages."
    )
    for parser_x in [
        parser_install,
        parser_reinstall,
        ...
        parser_x.add_argument(
            "--downloaddir", default=".", help="Specify download directory."
        )
        parser_x.add_argument(
            "--retries",
            default=5,
    ...
    parser_download.add_argument("templates", nargs="*", metavar="TEMPLATESPEC")

    # qvm-template {list,info}
    parser_list = parser_add_command("list", help_str="List templates.")
    parser_info = parser_add_command(
        "info", help_str="Display details about templates."
    )
    for parser_x in [parser_list, parser_info]:
        parser_x.add_argument(
            "--all", action="store_true", help="Show all templates (default)."
        )
        parser_x.add_argument(
            "--installed", action="store_true", help="Show installed templates."
        )
        parser_x.add_argument(
            "--available", action="store_true", help="Show available templates."
        )
        parser_x.add_argument(
            "--extras",
            action="store_true",
            help=(
                "Show extras (e.g., ones that exist"
                " locally but not in repos) templates."
            ),
        )
        parser_x.add_argument(
            "--upgrades", action="store_true", help="Show available upgrades."
        )
        parser_x.add_argument(
            "--all-versions",
            action="store_true",
    ...
    # qvm-template search
    parser_search = parser_add_command(
        "search", help_str="Search template details for the given string."
    )
    parser_search.add_argument(
        "--all",
        action="store_true",
        help=(
            "Search also in the template description and URL. In addition,"
    ...
    # qvm-template remove
    parser_remove = parser_add_command(
        "remove", help_str="Remove installed templates."
    )
    parser_remove.add_argument(
        "--disassoc",
        action="store_true",
        help=(
            "Also disassociate VMs from the templates to be removed."
    ...
    # qvm-template purge
    parser_purge = parser_add_command(
        "purge", help_str="Remove installed templates and associated VMs."
    )
    parser_purge.add_argument("templates", nargs="*", metavar="TEMPLATE")

    # qvm-template clean
    parser_clean = parser_add_command(
        "clean", help_str="Remove locally cached packages."
    )
    _ = parser_clean  # unused

    # qvm-template repolist
    parser_repolist = parser_add_command(
        "repolist", help_str="Show configured repositories."
    )
    repolim = parser_repolist.add_mutually_exclusive_group()
    repolim.add_argument("--all", action="store_true", help="Show all repos.")
    repolim.add_argument(
        "--enabled",
        action="store_true",
        help="Show only enabled repos (default).",
    )
    repolim.add_argument(
        "--disabled", action="store_true", help="Show only disabled repos."
    )
...
def is_match_spec(
    name: str, epoch: str, version: str, release: str, spec: str
...
def get_managed_template_vm(
    app: qubesadmin.app.QubesBase, name: str
...
def qrexec_popen(
    ...
    if args.updatevm:
        return app.domains[args.updatevm].run_service(
            service, filter_esc=filter_esc, stdout=stdout
        )
    # pylint: disable=consider-using-with
    return subprocess.Popen(
        [f"/etc/qubes-rpc/{service}"],
...
def qrexec_repoquery(
    ...
    for line in stdout.split("|\n"):
        ...
        try:
            ...
            if not is_match_spec(
                PACKAGE_NAME_PREFIX + name, epoch, version, release, spec
            )[0]:
                continue
            ...
            raise ConnectionError(
                "qrexec call 'qubes.TemplateSearch' failed:"
            ...
            with tqdm.tqdm(
                ...
                for i in rpmcanon.stdout:
                    if i.endswith(b" bytes so far\n"):
                        ...
                        raise ConnectionError(f"Bad line from rpmcanon: {i!r}")
...
def get_keys_for_repos(
    repo_files: typing.List[str], releasever: str
) -> typing.Dict[str, str]:
    """List gpg keys
...
def extract_rpm(name: str, path: str, target: str) -> bool:
    ...
    with open(path, "rb") as pkg_f:
        with subprocess.Popen(
            ["rpm2archive", "-"], stdin=pkg_f, stdout=subprocess.PIPE
        ) as rpm2archive:
            # `-D` is GNUism
            with subprocess.Popen(
                [
...
def filter_version(
    ...
    for entry in query_res:
        ...
        if version_selector == VersionSelector.LATEST:
            ...
            cur_ver = query_local_evr(vm)
            cmp_res = (
                -1 if version_selector == VersionSelector.LATEST_LOWER else 1
            )
            if rpm.labelCompare(evr, cur_ver) == cmp_res:
                if (
                    entry.name not in results
...
def get_dl_list(
    ...
    for template in args.templates:
        ...
        for entry in query_res:
            if (
                entry.name not in full_candid
                or rpm.labelCompare(full_candid[entry.name].evr, entry.evr) < 0
            ):
                full_candid[entry.name] = DlEntry(
                    entry.evr, entry.reponame, entry.dlsize
                )
    ...
    with tempfile.TemporaryDirectory(dir=path) as dl_dir:
        for name, entry in dl_list.items():
            ...
            if os.path.exists(target):
                print(
                    f"'{target}' already exists, skipping...", file=sys.stderr
                )
            ...
            for attempt in range(args.retries):
                try:
                    qrexec_download(
                        args, app, spec, target_temp, repo_key, entry.dlsize
                    )
                    done = True
                    break
                    ...
                    if attempt + 1 < args.retries:
                        print(
                            f"'{spec}' download failed, retrying...",
                            file=sys.stderr,
                        )
            if not done:
...
def install(
    ...
    def verify(rpmfile, reponame, package_hdr=None):
        ...
        if package_hdr is None:
            ...
            package_hdr = verify_rpm(
                rpmfile, repo_key, nogpgcheck=args.nogpgcheck
            )
            if not package_hdr:
        ...
        if not override_existing and name in app.domains:
            print(
                f"Template '{name}' already installed, skipping..."
        ...
        if override_existing:
            ...
            if version_selector == VersionSelector.REINSTALL and cmp_res != 0:
                parser.error(f"Same version of template '{name}' not found.")
            elif (
                version_selector == VersionSelector.LATEST_LOWER
                and cmp_res != -1
            ):
                print(
                    f"Template '{name}' of lower version "
                    f"already installed, skipping...",
                    file=sys.stderr,
                )
                return
            elif (
                version_selector == VersionSelector.LATEST_HIGHER
                and cmp_res != 1
            ):
                print(
                    f"Template '{name}' of higher version"
    ...
    for name, entry in dl_list.items():
        ...
        if not override_existing and name in app.domains:
            ...
            version_str = build_version_str(entry.evr)
            target_file = f"{PACKAGE_NAME_PREFIX}{name}-{version_str}.rpm"
            unverified_rpm_list[name] = (
                os.path.join(args.cachedir, target_file),
                entry.reponame,
            )
    ...
    for rpmfile, reponame, name, package_hdr in verified_rpm_list:
        with tempfile.TemporaryDirectory(dir=TEMP_DIR) as target:
            ...
            subprocess.check_call(
                cmdline
                + ["post-install", name, target + PATH_PREFIX + "/" + name]
            )
            ...
            tpl.features["template-epoch"] = package_hdr[rpm.RPMTAG_EPOCHNUM]
            tpl.features["template-version"] = package_hdr[rpm.RPMTAG_VERSION]
            tpl.features["template-release"] = package_hdr[rpm.RPMTAG_RELEASE]
            ...
            tpl.features["template-installtime"] = datetime.datetime.now(
                tz=datetime.timezone.utc
            ).strftime(DATE_FMT)
            tpl.features["template-license"] = package_hdr[rpm.RPMTAG_LICENSE]
            tpl.features["template-url"] = package_hdr[rpm.RPMTAG_URL]
            tpl.features["template-summary"] = package_hdr[rpm.RPMTAG_SUMMARY]
            tpl.features["template-description"] = package_hdr[
...
def list_templates(
    args: argparse.Namespace, app: qubesadmin.app.QubesBase, command: str
) -> None:
    ...
    def info_to_human_output(tpls):
        ...
        for status, grp in itertools.groupby(tpls, lambda x: x[0]):
            ...
            for _, data, install_time in grp:
                ...
                output.append(
                    ("Size", ":", qubesadmin.utils.size_to_human(data.dlsize))
                )
    ...
    def check_append(name, evr):
        return not args.templates or any(
            is_match_spec(name, *evr, spec)[0] for spec in args.templates
        )
    ...
    if args.all or args.available or args.extras or args.upgrades:
        if args.templates:
            ...
            for spec in args.templates:
                query_res_set |= set(
                    qrexec_repoquery(args, app, PACKAGE_NAME_PREFIX + spec)
                )
    ...
    if args.installed or args.all:
        for vm in app.domains:
            if is_managed_template(vm) and check_append(
                vm.name, query_local_evr(vm)
            ):
    ...
    if args.extras:
        ...
        for vm in app.domains:
            if (
                is_managed_template(vm)
                and vm.name not in remote
                and check_append(vm.name, query_local_evr(vm))
            ):
    ...
    if args.machine_readable:
        ...
        if command == "info":
            tpl_list_dict = info_to_machine_output(
                tpl_list, replace_newline=False
            )
        elif command == "list":
            tpl_list_dict = list_to_machine_output(tpl_list)
...
def search(args: argparse.Namespace, app: qubesadmin.app.QubesBase) -> None:
    ...
    search_res_by_idx: typing.Dict[
        int, typing.List[typing.Tuple[int, str, bool]]
    ] = collections.defaultdict(list)
    for keyword in args.templates:
        for idx, entry in enumerate(query_res):
            needle_types = [
                (entry.name, WEIGHT_NAME),
                (entry.summary, WEIGHT_SUMMARY),
            ]
            if args.all:
                needle_types += [
                    (entry.description, WEIGHT_DESCRIPTION),
...
def remove(
    ...
    if disassoc:
        ...
        cnt = 1
        while (
            dummy in app.domains
            and app.domains[dummy].features.get("template-dummy", "0") != "1"
        ):
        ...
        for tpl in remove_list:
            ...
            for holder, prop in qubesadmin.utils.vm_dependencies(app, vm):
                if holder:
                    ...
                    print(
                        f"Property '{prop}' of '{holder.name}' set to "
                        f"'{dummy}'.",
                        file=sys.stderr,
                    )
                else:
                    print(
                        f"Global property '{prop}' set to ''.", file=sys.stderr
                    )
                    setattr(app, prop, "")
        if remove_dummy:
        ...
        if args.repos:
            ...
            if repoid:
                if enable_disable_repos:
                    print(
                        "Warning: Ignoring --enablerepo and --disablerepo "
                        "options.",
                        file=sys.stderr,
                    )
...
def migrate_from_rpmdb(app):
    ...
    for pkg in rpm_ts.dbMatch():
        ...
        try:
            ...
            vm.features["template-description"] = pkg[
                rpm.RPMTAG_DESCRIPTION
            ].replace("\n", "|")
        ...
        pkgs_to_remove.append(pkg)
    subprocess.check_call(
        ["rpm", "-e", "--justdb"] + [p[rpm.RPMTAG_NAME] for p in pkgs_to_remove]
    )


def main(
    args: typing.Optional[typing.Sequence[str]] = None,
    app: typing.Optional[qubesadmin.app.QubesBase] = None,
) -> int:
    ...
    # resolve wildcards
    p_args.repo_files = list(
        itertools.chain(*(glob.glob(path) for path in p_args.repo_files))
    )
    ...
    try:
        ...
        if p_args.command == "download":
            ...
            install(
                p_args,
                app,
                version_selector=VersionSelector.REINSTALL,
