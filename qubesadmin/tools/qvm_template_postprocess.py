...
parser.add_argument("--pool", help="Specify pool to store created VMs in.")
parser.add_argument(
...
def import_root_img(vm, source_dir):
    ...
    if os.path.exists(root_path + ".part.00"):
        ...
        if (
            rpm2archive.returncode != 0
            or tar_parts.returncode != 0
            or tar_root_img.returncode != 0
        ):
            raise qubesadmin.exc.QubesException("root.img extraction failed")
    elif os.path.exists(root_path + ".tar"):
        with subprocess.Popen(
            ["tar", "xSOf", root_path + ".tar"], stdout=subprocess.PIPE
        ) as tar:
            vm.volumes["root"].import_data_with_size(
...
def import_appmenus(vm, source_dir, skip_generate=True):
    ...
    try:
        with open(
            source_dir / "vm-whitelisted-appmenus.list", "r", encoding="ascii"
        ) as fd:
            vm.features["default-menu-items"] = " ".join(
                [x.rstrip() for x in fd]
            )
    except FileNotFoundError as e:
        vm.log.warning(
            "Cannot set default-menu-items, %s not found", e.filename
        )
    try:
        with open(
            source_dir / "whitelisted-appmenus.list", "r", encoding="ascii"
        ) as fd:
            vm.features["menu-items"] = " ".join([x.rstrip() for x in fd])
    except FileNotFoundError as e:
        vm.log.warning("Cannot set menu-items, %s not found", e.filename)
    try:
        with open(
            source_dir / "netvm-whitelisted-appmenus.list",
            "r",
            encoding="ascii",
        ) as fd:
            vm.features["netvm-menu-items"] = " ".join([x.rstrip() for x in fd])
    except FileNotFoundError as e:
        vm.log.warning("Cannot set netvm-menu-items, %s not found", e.filename)
        ...
        subprocess.check_call(
            ...
            + [
                ...
                "--set-default-whitelist={!s}".format(
                    source_dir / "vm-whitelisted-appmenus.list"
                ),
                vm.name,
            ]
        )
        subprocess.check_call(
            ...
            + [
                ...
                "--set-whitelist={!s}".format(
                    source_dir / "whitelisted-appmenus.list"
                ),
                vm.name,
            ]
        )
...
def import_template_config(args, conf_path, vm):
    ...
    for key in (
        ...
        if key in conf:
            if conf[key] == "1":
                ...
                vm.log.warning("ignoring boolean config flags that are not '1'")
    for key in ("net.fake-ip", "net.fake-gateway", "net.fake-netmask"):
        if key in conf:
            if validate_ip(conf[key]):
                vm.features[key] = conf[key]
            else:
                vm.log.warning("ignoring invalid value for '%s'", key)
    ...
    if "kernel" in conf:
        if conf["kernel"] == "":
            ...
            vm.log.warning("Currently only supports setting kernel to (none)")
