...
def retrieve_gui_daemon_options(vm, guivm):
    ...
    for name, kind in GUI_DAEMON_OPTIONS:
        feature_value = vm.features.get("gui-" + name.replace("_", "-"), None)
...
class DAEMONLauncher:
    ...
    def on_domain_start(self, vm, _event, **kwargs):
        ...
        try:
            if (
                getattr(vm, "audiovm", None) == vm.app.local_name
...
def main(args=None):
    ...
    if (
        ...
        and not os.path.exists("/etc/qubes-release")
    ):
        ...
        with os.fdopen(fd, "r+") as lock_f:
            try:
                ...
                try:
                    ...
                    pid = "unknown"
                print(
                    "Another GUI daemon process (with PID {}) is already "
                    "running".format(pid),
                    file=sys.stderr,
                )
                sys.exit(1)
