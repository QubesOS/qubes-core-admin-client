...
class RestoreInDisposableVM:
    ...
    arguments = {
        ...
        "rename_conflicting": Option(
            ("--rename-conflicting",), handle_store_true
        ),
        "exclude": Option(("--exclude", "-x"), handle_append),
        "dom0_home": Option(("--skip-dom0-home",), handle_store_false),
        "ignore_username_mismatch": Option(
            ("--ignore-username-mismatch",), handle_store_true
        ),
        "ignore_size_limit": Option(
            ("--ignore-size-limit",), handle_store_true
        ),
        ...
        "location_is_service": Option(
            ("--location-is-service",), handle_store_true
        ),
        "paranoid_mode": Option(
            (
                "--paranoid-mode",
                "--plan-b",
            ),
    ...
    def __init__(self, app, args):
        ...
        self.terminal_app = (
            ...
            "/bin/sh",
            "-c",
            '("$0" "$@" 2>&1; echo exit code: $?) | tee {}'.format(
                self.backup_log_path
            ),
        )
        if args.auto_close:
            # filter-out '-hold'
            self.terminal_app = tuple(
                a for a in self.terminal_app if a != "-hold"
            )
    ...
    def transfer_pass_file(self, path):
        ...
        return "/home/{}/QubesIncoming/{}/{}".format(
            self.dispvm.default_user, os.uname()[1], os.path.basename(path)
        )
    ...
    def register_backup_source(self):
        ...
        self.storage_access_proc.stdin.write(
            (
                self.args.backup_location.replace("\r", "").replace("\n", "")
                + "\n"
            ).encode()
        )
        ...
        if not storage_access_id or not all(
            c in allowed_chars for c in storage_access_id
        ):
    ...
    def finalize_tags(self):
        ...
        for domain in self.app.domains:
            if "backup-restore-in-progress" not in domain.tags:
                continue
            if not any(
                t.startswith("restored-from-backup-") for t in domain.tags
            ):
                self.log.warning(
                    "Restored domain %s was not tagged with "
                    "restored-from-backup-* tag",
                    domain.name,
                )
                # add fallback tag
                domain.tags.add(
                    "restored-from-backup-at-{}".format(
        ...
        try:
            ...
            if e.returncode == 127:
                raise qubesadmin.exc.QubesException(
                    ...
                    "package there".format(
                        self.terminal_app[0], self.dispvm.template.template.name
                    )
                )
            try:
                backup_log = self.extract_log()
