...
class MockVolume(qubesadmin.storage.Volume):
    def __init__(self, import_data_queue, delay_stream, *args, **kwargs):
        ...
        self.app = AppProxy(
            self.app, import_data_queue, delay_stream=delay_stream
        )
...
class TC_10_BackupCompatibility(qubesadmin.tests.backup.BackupTestCase):
    ...
    def create_private_img(self, filename, sparse=True):
        ...
        if sparse:
            self.create_sparse(
                filename, 2 * 2**20, signature=signature.encode()
            )
        else:
            self.create_full_image(
                filename, 2 * 2**30, signature=signature.encode()
            )
    ...
    def handle_v4_file(self, f_name, subdir, stream, compressed="gzip"):
        ...
        for part in sorted(os.listdir(stage1_dir)):
            if not re.match(
                ...
                continue
            part_with_dir = os.path.join(subdir, part)
            f_name_enc = self.scrypt_encrypt(
                part_with_dir, basedir=self.fullpath("stage1")
            )
            self.append_backup_stream(
                f_name_enc, stream, basedir=self.fullpath("stage1")
            )
    ...
    def create_v4_backup(self, compressed="gzip", big=False):
        ...
        if big:
            ...
            self.create_private_img(
                self.fullpath("appvms/test-work/private.img"), sparse=False
            )
        ...
        for name, vm in parsed_qubes_xml["domains"].items():
            ...
            for tag in vm["tags"]:
                if tag.startswith("created-by-"):
                    ...
                    self.app.expected_calls[
                        (name, "admin.vm.tag.Get", tag, None)
                    ] = b"0\0001"
                else:
                    self.app.expected_calls[
                        (name, "admin.vm.tag.Set", tag, None)
            ...
            if vm["backup_path"]:
                ...
                self.app.expected_calls[(name, "appmenus", None, appmenus)] = (
                    b"0\0"
                )
