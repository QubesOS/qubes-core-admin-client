...
class TC_00_DeviceCollection(qubesadmin.tests.QubesTestCase):
    ...
    def test_030_detach(self):
        self.app.expected_calls[
            ("test-vm", "admin.vm.device.test.Detach", "test-vm2+dev1", None)
        ] = b"0\0"
        assign = qubesadmin.devices.DeviceAssignment(
            self.app.domains["test-vm2"], "dev1"
        )
    ...
    def test_040_assignments(self):
        ...
        self.assertEqual(
            assigns[0].backend_domain, self.app.domains["test-vm2"]
        )
        self.assertEqual(assigns[0].ident, "dev1")
        self.assertEqual(
            assigns[0].frontend_domain, self.app.domains["test-vm"]
        )
        ...
        self.assertEqual(
            assigns[1].backend_domain, self.app.domains["test-vm3"]
        )
        self.assertEqual(assigns[1].ident, "dev2")
        self.assertEqual(
            assigns[1].frontend_domain, self.app.domains["test-vm"]
        )
    ...
    def test_041_assignments_persistent(self):
        ...
        self.assertIsInstance(assigns[0], qubesadmin.devices.DeviceAssignment)
        self.assertEqual(
            assigns[0].backend_domain, self.app.domains["test-vm3"]
        )
    ...
    def test_070_update_persistent(self):
        self.app.expected_calls[
            (
                "test-vm",
                "admin.vm.device.test.Set.persistent",
                "test-vm2+dev1",
                b"True",
            )
        ] = b"0\0"
