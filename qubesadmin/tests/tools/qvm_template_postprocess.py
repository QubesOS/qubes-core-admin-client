...
class TC_00_qvm_template_postprocess(qubesadmin.tests.QubesTestCase):
    ...
    def test_001_import_root_img_tar(self):
        ...
        subprocess.check_call(
            [
                ...
                "--clean",
                "-D",
                f"_topdir {self.source_dir.name}",
                spec,
            ],
            cwd=self.source_dir.name,
    ...
    def test_005_reset_private_img(self):
        ...
        self.app.expected_calls[
            ("test-vm", "admin.vm.volume.Clear", "private", None)
        ] = b"0\0"
    ...
    def test_010_import_appmenus(self):
        default_menu_items = ["org.gnome.Terminal.desktop", "firefox.desktop"]
        menu_items = [
        ...
        with open(
            os.path.join(
                self.source_dir.name, "netvm-whitelisted-appmenus.list"
            ),
            "w",
        ) as f:
        ...
        self.app.expected_calls[
            (
                ...
                "default-menu-items",
                " ".join(default_menu_items).encode(),
            )
        ] = b"0\0"
        self.app.expected_calls[
            (
    ...
    def test_020_post_install(self, mock_import_root_img, mock_import_appmenus):
        ...
        self.app.expected_calls[("test-vm", "admin.vm.Start", None, None)] = (
            b"0\0"
        )
        self.app.expected_calls[
        ...
        self.app.add_new_vm.assert_called_once_with(
            "TemplateVM", name="test-vm", label="black", pool=None
        )
...
"""
        ...
        self.app.expected_calls[
            ("test-vm", "admin.vm.feature.Set", "gui", b"1")
        ] = b"0\0"
        self.app.expected_calls[
            ("test-vm", "admin.vm.feature.Set", "qrexec", b"1")
        ] = b"0\0"
        self.app.expected_calls[
            ("test-vm", "admin.vm.feature.Set", "linux-stubdom", b"1")
        ] = b"0\0"
        self.app.expected_calls[
            ("test-vm", "admin.vm.feature.Set", "net.fake-ip", b"192.168.1.100")
        ] = b"0\0"
        self.app.expected_calls[
            (
                "test-vm",
        ...
        self.app.expected_calls[
            ("test-vm", "admin.vm.property.Set", "virt_mode", b"hvm")
        ] = b"0\0"
        self.app.expected_calls[
            ("test-vm", "admin.vm.property.Set", "kernel", b"")
        ] = b"0\0"
    ...
    def test_052_template_config_virt_mode_pv(self):
        ...
        with self.subTest("allowed"):
            ...
            self.app.expected_calls[
                ("test-vm", "admin.vm.property.Set", "virt_mode", b"pv")
            ] = b"0\0"
            template_config = "virt-mode=pv\n"
            template_conf = os.path.join(self.source_dir.name, "template.conf")
            with open(template_conf, "w") as f:
