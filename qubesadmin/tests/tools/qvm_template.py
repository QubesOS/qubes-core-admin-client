...
"""
        ...
        cb(
            os.path.join(d, "pub.asc"),
            os.path.join(
                d, "RPMS/noarch/qubes-template-invalid-0.0.0-1.noarch.rpm"
            ),
        )
...
class TC_00_qvm_template(qubesadmin.tests.QubesTestCase):
    ...
    def test_000_verify_rpm_success(self, mock_proc, mock_call, mock_ts):
        ...
        ret = qubesadmin.tools.qvm_template.verify_rpm(
            "/dev/null", "/path/to/key", template_name="test-vm"
        )
    ...
    def test_001_verify_rpm_nosig_fail(self, mock_proc, mock_call, mock_ts):
        ...
        with self.assertRaises(Exception) as e:
            qubesadmin.tools.qvm_template.verify_rpm(
                "/dev/null", "/path/to/key"
            )
        mock_call.assert_called_once()
        mock_proc.assert_called_once()
    ...
    def test_002_verify_rpm_nosig_success(self, mock_proc, mock_call, mock_ts):
        ...
        mock_proc.return_value = b"-: digests OK\n"
        ret = qubesadmin.tools.qvm_template.verify_rpm(
            "/dev/null", "/path/to/key", nogpgcheck=True
        )
    ...
    def test_003_verify_rpm_badsig_fail(self, mock_proc, mock_call, mock_ts):
        mock_proc.side_effect = subprocess.CalledProcessError(
            1, ["rpmkeys", "--checksig"], b"-: digests SIGNATURES NOT OK\n"
        )
        with self.assertRaises(Exception) as e:
            qubesadmin.tools.qvm_template.verify_rpm(
                "/dev/null", "/path/to/key"
    ...
    def test_004_verify_rpm_badname(self, mock_proc, mock_call, mock_ts):
        ...
        mock_proc.assert_called_once()
        self.assertIn(
            "package does not match expected template name", e.exception.args[0]
        )
    ...
    def test_010_extract_rpm_success(self, mock_popen, mock_path_exists):
        ...
        with tempfile.NamedTemporaryFile() as fd, tempfile.TemporaryDirectory() as dir:
            path = fd.name
            dirpath = dir
        ...
        self.assertEqual(
            ...
            [
                mock.call(
                    ["rpm2archive", "-"], stdin=mock.ANY, stdout=subprocess.PIPE
                ),
                mock.call().__enter__(),
                mock.call(
                    [
        ...
        for failing_call in range(1, 5):
            ...
            with self.subTest(failing_call=failing_call):
                ...
                def side_effect(_, **__):
                    ...
                    o.stdout = pipe
                    o.returncode = (
                        1 if side_effect.call_count >= failing_call else 0
                    )
                    return o
                ...
                with tempfile.NamedTemporaryFile() as fd, tempfile.TemporaryDirectory() as tmpdir:
                    path = fd.name
                    dirpath = tmpdir
                    ret = qubesadmin.tools.qvm_template.extract_rpm(
                        "test-vm", path, dirpath
    ...
    def test_090_install_lock(self, mock_get_keys):
        ...
        with mock.patch(
            "qubesadmin.tools.qvm_template.LOCK_FILE", "/tmp/test.lock"
        ):
            with self.subTest("not locked"):
                with self.assertRaises(SuccessError):
                    # args don't matter
                    qubesadmin.tools.qvm_template.install(
                        mock.MagicMock(), None
                    )
    ...
    def test_100_install_local_success(
        ...
        mock_time.now.return_value = datetime.datetime(
            2020, 9, 1, 15, 30, tzinfo=datetime.timezone.utc
        )
        with mock.patch(
            "qubesadmin.tools.qvm_template.LOCK_FILE", "/tmp/test.lock"
        ), mock.patch("datetime.datetime", new=mock_time), mock.patch(
            "tempfile.TemporaryDirectory"
        ) as mock_tmpdir, mock.patch(
            "sys.stderr", new=io.StringIO()
        ) as mock_err, tempfile.NamedTemporaryFile(
            suffix=".rpm"
        ) as template_file:
        ...
        mock_verify.assert_called_once_with(
            template_file.name, "/tmp/keyring.gpg", nogpgcheck=False
        )
        # Package is extracted
        mock_extract.assert_called_with(
            "test-vm", path, "/var/tmp/qvm-template-tmpdir"
        )
        ...
        self.assertEqual(
            mock_mkdirs.mock_calls, [mock.call(args.cachedir, exist_ok=True)]
        )
    ...
    def test_106_install_local_badpath_fail(
        ...
        with mock.patch(
            ...
            # Check error message
            self.assertTrue(
                f"RPM file '{path}' not found" in mock_err.getvalue()
            )
        # Should not be executed:
    ...
    def test_110_qrexec_payload_refresh_success(self):
        with tempfile.NamedTemporaryFile() as repo_conf1, tempfile.NamedTemporaryFile() as repo_conf2:
            repo_str1 = """[qubes-templates-itl]
...
"""
            repo_str2 = """[qubes-templates-itl-testing]
name = Qubes Templates repository
...
"""
            ...
            res = qubesadmin.tools.qvm_template.qrexec_payload(
                args, self.app, "qubes-template-fedora-32", True
            )
            self.assertEqual(
                res,
                """--enablerepo=repo1
...
"""
            ...
            res = qubesadmin.tools.qvm_template.qrexec_payload(
                args, self.app, "qubes-template-fedora-32", False
            )
            self.assertEqual(
                res,
                """--repoid=repo1
...
"""
            ...
            with mock.patch("sys.stderr", new=io.StringIO()) as mock_err:
                with self.assertRaises(SystemExit):
                    qubesadmin.tools.qvm_template.qrexec_payload(
                        args, self.app, "qubes-template-fedora\n-32", False
                    )
                # Check error message
                self.assertTrue(
                    "Malformed template name" in mock_err.getvalue()
                )
                self.assertTrue(
...
"""
            ...
            with mock.patch("sys.stderr", new=io.StringIO()) as mock_err:
                ...
                # Check error message
                self.assertTrue("Malformed --enablerepo" in mock_err.getvalue())
                self.assertTrue(
                    "argument should not contain '\\n'" in mock_err.getvalue()
                )
        self.assertAllCalled()
...
"""
            ...
            with mock.patch("sys.stderr", new=io.StringIO()) as mock_err:
                with self.assertRaises(SystemExit):
                    ...
                    # Check error message
                    self.assertTrue(
                        "Malformed --disablerepo" in mock_err.getvalue()
                    )
                    self.assertTrue(
                        "argument should not contain '\\n'"
                        in mock_err.getvalue()
...
"""
            ...
            with mock.patch("sys.stderr", new=io.StringIO()) as mock_err:
                ...
                # Check error message
                self.assertTrue("Malformed --repoid" in mock_err.getvalue())
                self.assertTrue(
                    "argument should not contain '\\n'" in mock_err.getvalue()
                )
        self.assertAllCalled()
...
"""
            ...
            with mock.patch("sys.stderr", new=io.StringIO()) as mock_err:
                ...
                # Check error message
                self.assertTrue("Malformed --releasever" in mock_err.getvalue())
                self.assertTrue(
                    "argument should not contain '\\n'" in mock_err.getvalue()
                )
        self.assertAllCalled()
...
"""
            ...
            with mock.patch("sys.stderr", new=io.StringIO()) as mock_err:
                ...
                self.assertTrue(
                    "argument should not be '---'" in mock_err.getvalue()
                )
        self.assertAllCalled()
...
"""
        res = qubesadmin.tools.qvm_template.qrexec_repoquery(
            args, self.app, "qubes-template-fedora-32"
        )
        self.assertEqual(
            res,
            [
...
"""
    ...
    def test_124_qrexec_repoquery_searchfail_fail(self, mock_payload):
        ...
        with mock.patch("qubesadmin.tests.TestProcess.wait") as mock_wait:
            mock_wait.return_value = 1
            with self.assertRaises(ConnectionError):
                qubesadmin.tools.qvm_template.qrexec_repoquery(
                    args, self.app, "qubes-template-fedora-32"
                )
        self.assertEqual(
            self.app.service_calls,
...
qubes-template-fedora-32|0|4.1|20200101|qubes-templates-itl|1048576|2020-01-23 04:56|GPL|https://qubes-os.org|Qubes template for fedora-32|Qubes template for fedora-32\n|
"""
        with self.assertRaisesRegex(ConnectionError, "unexpected data format"):
            qubesadmin.tools.qvm_template.qrexec_repoquery(
                args, self.app, "qubes-template-fedora-32"
            )
        self.assertEqual(
...
"""
    ...
    def test_133_get_dl_list_reinstall_success(self, mock_query):
        ...
        self.app.expected_calls[
            ("test-vm", "admin.vm.feature.Get", f"template-name", None)
        ] = b"0\0test-vm"
        self.app.expected_calls[
            ("test-vm", "admin.vm.feature.Get", f"template-epoch", None)
        ] = b"0\x000"
        self.app.expected_calls[
            ("test-vm", "admin.vm.feature.Get", f"template-version", None)
        ] = b"0\x004.2"
        self.app.expected_calls[
            ("test-vm", "admin.vm.feature.Get", f"template-release", None)
        ] = b"0\x0020200201"
        mock_query.return_value = [
            qubesadmin.tools.qvm_template.Template(
                "test-vm",
        ...
        args = argparse.Namespace(templates=["some.local.file.rpm", "test-vm"])
        ret = qubesadmin.tools.qvm_template.get_dl_list(
            args,
            self.app,
    ...
    def test_134_get_dl_list_reinstall_nolocal_fail(self, mock_query):
        ...
        with mock.patch("sys.stderr", new=io.StringIO()) as mock_err:
            with self.assertRaises(SystemExit):
                qubesadmin.tools.qvm_template.get_dl_list(
                    args,
                    self.app,
                    qubesadmin.tools.qvm_template.VersionSelector.REINSTALL,
                )
    ...
    def test_135_get_dl_list_reinstall_nonmanaged_fail(self, mock_query):
        ...
        with mock.patch(
            "sys.stderr", new=io.StringIO()
        ) as mock_err, mock.patch.object(self.app, "qubesd_call") as mock_call:
    ...
    def test_150_list_templates_installed_success(self, mock_query):
        ...
        for key, val in [
            ...
            self.app.expected_calls[
                ("test-vm", "admin.vm.feature.Get", f"template-{key}", None)
            ] = (b"0\0" + val.encode())
        for key, val in [("name", "test-vm-2-not-managed")]:
            self.app.expected_calls[
                ("test-vm-2", "admin.vm.feature.Get", f"template-{key}", None)
            ] = (b"0\0" + val.encode())
        for key, val in [
            ("name", "non-spec"),
            ("epoch", "0"),
            ...
            self.app.expected_calls[
                ("non-spec", "admin.vm.feature.Get", f"template-{key}", None)
            ] = (b"0\0" + val.encode())
        args = argparse.Namespace(
            all=False,
            installed=True,
        ...
        ) as mock_out, mock.patch.object(
            self.app.domains["test-vm"], "get_disk_utilization"
        ) as mock_disk:
            mock_disk.return_value = 1234321
            qubesadmin.tools.qvm_template.list_templates(args, self.app, "list")
            self.assertEqual(
                mock_out.getvalue(),
                """Installed Templates
...
""",
        ...
        self.assertEqual(mock_query.mock_calls, [mock.call(args, self.app)])
        self.assertAllCalled()
    ...
    def test_152_list_templates_extras_success(self, mock_query):
        mock_query.return_value = [
            qubesadmin.tools.qvm_template.Template(
                ...
                1048576,
                datetime.datetime(
                    2020, 9, 1, 14, 30, tzinfo=datetime.timezone.utc
                ),
                "GPL",
                "https://qubes-os.org",
                "Qubes template for fedora-31",
        ...
        for key, val in [("name", "test-vm-3-non-managed")]:
            self.app.expected_calls[
                ("test-vm-3", "admin.vm.feature.Get", f"template-{key}", None)
            ] = (b"0\0" + val.encode())
        for key, val in [
            ("name", "non-spec"),
            ("epoch", "1"),
        ...
        with mock.patch(
            ...
            self.app.domains["test-vm-2"], "get_disk_utilization"
        ) as mock_disk:
            mock_disk.return_value = 1234321
            qubesadmin.tools.qvm_template.list_templates(args, self.app, "list")
            self.assertEqual(
...
""",
    ...
    @mock.patch("qubesadmin.tools.qvm_template.qrexec_repoquery")
    def __test_list_templates_all_success(
        self, operation, args, expected, mock_query
    ):
        mock_query.return_value = [
            qubesadmin.tools.qvm_template.Template(
                "test-vm",
...
"""
    ...
    def test_157_list_templates_list_all_machinereadable_success(self):
        ...
        expected = """installed|test-vm-2|1:4.0-2019|@commandline
...
"""
    ...
    def test_158_list_templates_info_all_machinereadable_success(self):
        ...
        expected = """installed|test-vm-2|1|4.0|2019|@commandline|1234321|2020-09-02 14:30:00|2020-09-02 15:30:00|GPL|https://qubes-os.org|Summary|Desc|desc
...
"""
    ...
    def test_159_list_templates_list_all_machinereadablejson_success(self):
        ...
        expected = """{"installed": [{"name": "test-vm-2", "evr": "1:4.0-2019", "reponame": "@commandline"}], "available": [{"name": "test-vm", "evr": "2:4.1-2020", "reponame": "qubes-templates-itl"}]}
"""
        self.__test_list_templates_all_success("list", args, expected)
...
"""
    ...
    def test_170_search_success(self, mock_query):
        ...
        args = argparse.Namespace(all=False, templates=["test-vm"])
        with mock.patch(
            "sys.stdout", new=io.StringIO()
        ) as mock_out, mock.patch.object(
            self.app.domains["test-vm-2"], "get_disk_utilization"
...
""",
    ...
    def test_173_search_multiquery_success(self, mock_query):
        ...
        args = argparse.Namespace(
            all=False, templates=["test-vm", "test-template"]
        )
        with mock.patch("sys.stdout", new=io.StringIO()) as mock_out:
            qubesadmin.tools.qvm_template.search(args, self.app)
            self.assertEqual(
...
""",
    ...
    def test_174_search_multiquery_exact_success(self, mock_query):
        ...
        args = argparse.Namespace(all=False, templates=["test-vm", "summary"])
        with mock.patch("sys.stdout", new=io.StringIO()) as mock_out:
            qubesadmin.tools.qvm_template.search(args, self.app)
            self.assertEqual(
                mock_out.getvalue(),
...
""",
    ...
    def test_176_search_wildcard_success(self, mock_query):
        ...
        args = argparse.Namespace(all=False, templates=["t?st-vm"])
        with mock.patch("sys.stdout", new=io.StringIO()) as mock_out:
            qubesadmin.tools.qvm_template.search(args, self.app)
            self.assertEqual(
                mock_out.getvalue(),
...
""",
    ...
    def _mock_qrexec_download(
        self, args, app, spec, path, key, dlsize=None, refresh=False
    ):
        self.assertFalse(
            os.path.exists(path), "{} should not exist before".format(path)
        )
    ...
    def test_180_download_success(
        self, mock_qrexec, mock_dllist, mock_verify_rpm
    ):
        ...
        with tempfile.TemporaryDirectory() as dir:
            ...
            pkgs = qubesadmin.tools.qvm_template.download(
                ...
                dir,
                {
                    ...
                    "fedora-32": qubesadmin.tools.qvm_template.DlEntry(
                        ("0", "1", "2"), "qubes-templates-itl-testing", 2048576
                    ),
                },
            )
            ...
            self.assertEqual(
                mock_qrexec.mock_calls,
                [
                    mock.call(
                        args,
                        self.app,
                        "qubes-template-fedora-31-1:2-3",
                        re_str(
                            dir
                            + "/.*/qubes-template-fedora-31-1:2-3.rpm.UNTRUSTED"
                        ),
                        "/tmp/keyring.gpg",
                        1048576,
                    ),
                    mock.call(
                        args,
                        self.app,
                        "qubes-template-fedora-32-0:1-2",
                        re_str(
                            dir
                            + "/.*/qubes-template-fedora-32-0:1-2.rpm.UNTRUSTED"
                        ),
                        "/tmp/keyring.gpg",
                        2048576,
                    ),
    ...
    @mock.patch("qubesadmin.tools.qvm_template.get_dl_list")
    @mock.patch("qubesadmin.tools.qvm_template.qrexec_download")
    def test_181_download_success_nosuffix(
        self, mock_qrexec, mock_dllist, mock_verify_rpm
    ):
        mock_qrexec.side_effect = self._mock_qrexec_download
        with tempfile.TemporaryDirectory() as dir:
    ...
    @mock.patch("qubesadmin.tools.qvm_template.get_dl_list")
    @mock.patch("qubesadmin.tools.qvm_template.qrexec_download")
    def test_182_download_success_getdllist(
        self, mock_qrexec, mock_dllist, mock_verify_rpm
    ):
        mock_qrexec.side_effect = self._mock_qrexec_download
        ...
        with tempfile.TemporaryDirectory() as dir:
            ...
            self.assertEqual(
                mock_dllist.mock_calls,
                [
                    mock.call(
                        args,
                        self.app,
                        version_selector=qubesadmin.tools.qvm_template.VersionSelector.LATEST_LOWER,
                    )
                ],
            )
            self.assertEqual(
    ...
    @mock.patch("qubesadmin.tools.qvm_template.get_dl_list")
    @mock.patch("qubesadmin.tools.qvm_template.qrexec_download")
    def test_183_download_success_downloaddir(
        self, mock_qrexec, mock_dllist, mock_verify_rpm
    ):
        mock_qrexec.side_effect = self._mock_qrexec_download
        with tempfile.TemporaryDirectory() as dir:
    ...
    @mock.patch("qubesadmin.tools.qvm_template.get_dl_list")
    @mock.patch("qubesadmin.tools.qvm_template.qrexec_download")
    def test_184_download_success_exists(
        self, mock_qrexec, mock_dllist, mock_verify_rpm
    ):
        mock_qrexec.side_effect = self._mock_qrexec_download
        with tempfile.TemporaryDirectory() as dir:
            with open(
                os.path.join(dir, "qubes-template-fedora-31-1:2-3.rpm"), "w"
            ) as _:
                pass
            args = argparse.Namespace(
                retries=1,
            ...
            with mock.patch("sys.stderr", new=io.StringIO()) as mock_err:
                ...
                self.assertIn("fedora-32", pkgs)
                self.assertTrue(
                    "already exists, skipping" in mock_err.getvalue()
                )
            self.assertEqual(
                mock_verify_rpm.mock_calls,
                [
    ...
    @mock.patch("qubesadmin.tools.qvm_template.get_dl_list")
    @mock.patch("qubesadmin.tools.qvm_template.qrexec_download")
    def test_185_download_success_existsmove(
        self, mock_qrexec, mock_dllist, mock_verify_rpm
    ):
        mock_qrexec.side_effect = self._mock_qrexec_download
        with tempfile.TemporaryDirectory() as dir:
            ...
            self.assertEqual(mock_dllist.mock_calls, [])
            self.assertTrue(
                os.path.exists(dir + "/qubes-template-fedora-31-1:2-3.rpm")
            )

    @mock.patch("qubesadmin.tools.qvm_template.verify_rpm")
    @mock.patch("qubesadmin.tools.qvm_template.get_dl_list")
    @mock.patch("qubesadmin.tools.qvm_template.qrexec_download")
    def test_186_download_success_existsnosuffix(
        self, mock_qrexec, mock_dllist, mock_verify_rpm
    ):
        mock_qrexec.side_effect = self._mock_qrexec_download
        with tempfile.TemporaryDirectory() as dir:
    ...
    @mock.patch("qubesadmin.tools.qvm_template.get_dl_list")
    @mock.patch("qubesadmin.tools.qvm_template.qrexec_download")
    def test_187_download_success_retry(
        self, mock_qrexec, mock_dllist, mock_verify_rpm
    ):
        counter = 0
        ...
        with tempfile.TemporaryDirectory() as dir:
            ...
            with mock.patch(
                "sys.stderr", new=io.StringIO()
            ) as mock_err, mock.patch("os.remove") as mock_rm:
                pkgs = qubesadmin.tools.qvm_template.download(
                    args,
                    self.app,
                    None,
    ...
    @mock.patch("qubesadmin.tools.qvm_template.get_dl_list")
    @mock.patch("qubesadmin.tools.qvm_template.qrexec_download")
    def test_188_download_fail_retry(
        self, mock_qrexec, mock_dllist, mock_verify_rpm
    ):
        mock_qrexec.side_effect = self._mock_qrexec_download
        counter = 0
    ...
    @mock.patch("qubesadmin.tools.qvm_template.get_dl_list")
    @mock.patch("qubesadmin.tools.qvm_template.qrexec_download")
    def test_189_download_fail_interrupt(
        self, mock_qrexec, mock_dllist, mock_verify_rpm
    ):
        def f(*args):
            raise RuntimeError
    ...
    @mock.patch("qubesadmin.tools.qvm_template.get_dl_list")
    @mock.patch("qubesadmin.tools.qvm_template.qrexec_download")
    def test_190_download_fail_verify(
        self, mock_qrexec, mock_dllist, mock_verify_rpm
    ):
        mock_qrexec.side_effect = self._mock_qrexec_download
    ...
    @mock.patch("qubesadmin.tools.qvm_template.get_dl_list")
    @mock.patch("qubesadmin.tools.qvm_template.qrexec_download")
    def test_191_download_fail(self, mock_qrexec, mock_dllist, mock_verify_rpm):
        def f(*args, **kwargs):
            raise ConnectionError
    ...
    def _mock_qrexec_download_short(
        self, args, app, spec, path, key, dlsize=None, refresh=False
    ):
        self.assertFalse(
            os.path.exists(path), "{} should not exist before".format(path)
    ...
    def test_200_reinstall_success(
        ...
        # Downloaded package should not be removed
        self.assertEqual(mock_remove.mock_calls, [mock.call("/tmp/test.lock")])
        self.assertAllCalled()
    ...
    def test_201_reinstall_fail_noversion(
        ...
        with mock.patch(
            "qubesadmin.tools.qvm_template.LOCK_FILE", "/tmp/test.lock"
        ), self.assertRaises(SystemExit) as e, mock.patch(
            "sys.stderr", new=io.StringIO()
        ) as mock_err:
            args = argparse.Namespace(
                templates=["test-vm"],
    ...
    def test_202_reinstall_local_success(
        ...
        with mock.patch(
            ...
            "tempfile.TemporaryDirectory"
        ) as mock_tmpdir, tempfile.NamedTemporaryFile(
            suffix=".rpm"
        ) as template_file:
            path = template_file.name
            args = argparse.Namespace(
    ...
    def test_210_remove_success(self, mock_confirm, mock_remove):
        ...
        args = argparse.Namespace(templates=["vm1", "vm2"], yes=False)
    ...
    def test_211_remove_purge_disassoc_success(
        self, mock_confirm, mock_remove, mock_kill
    ):
        self.app.expected_calls[("dom0", "admin.vm.List", None, None)] = (
            b"0\x00vm1 class=TemplateVM state=Halted\n"
            b"vm2 class=TemplateVM state=Halted\n"
        ...
        self.app.expected_calls[
            ("dummy-1", "admin.vm.feature.Get", "template-dummy", None)
        ] = b"0\x001"
        self.app.expected_calls[
            ("vm2", "admin.vm.property.Set", "default_template", b"dummy-1")
        ] = b"0\x00"
        self.app.expected_calls[
        ...
        args = argparse.Namespace(templates=["vm1"], yes=False)

        def deps(app, vm):
            if vm == "vm1":
                return [
                    (self.app.domains["vm2"], "default_template"),
        ...
        self.assertEqual(
            ...
            [
                mock.call(
                    ["--", "vm1", "vm2", "vm3", "vm4", "dummy-1"], self.app
                )
            ],
        )
        self.assertAllCalled()
    ...
    def test_212_remove_disassoc_success(
        ...
        args = argparse.Namespace(
            templates=["vm1", "vm2", "vm3", "vm4"], yes=False
        )
        ...
        self.assertEqual(
            ...
            [
                mock.call(
                    ["--force", "--", "vm1", "vm2", "vm3", "vm4"], self.app
                )
            ],
        )
        self.assertEqual(
    ...
    def test_213_remove_fail_nodomain(self):
        ...
        args = argparse.Namespace(templates=["vm0"], yes=False)
        with mock.patch("sys.stderr", new=io.StringIO()) as mock_err:
            with self.assertRaises(SystemExit):
                qubesadmin.tools.qvm_template.remove(args, self.app)
    ...
    def test_214_remove_disassoc_success_newdummy(
        ...
        self.app.expected_calls[
            ("dummy-1", "admin.vm.feature.Set", "template-dummy", b"1")
        ] = b"0\x00"
        self.app.expected_calls[
            ("vm2", "admin.vm.property.Set", "default_template", b"dummy-1")
        ] = b"0\x00"
        ...
        self.assertEqual(
            mock_kill.mock_calls, [mock.call(["--", "vm1"], self.app)]
        )
        self.assertAllCalled()
...
"""
            ...
            self.assertEqual(
                ret,
                {
                    "qubes-templates-itl": "/etc/qubes/repo-templates/keys/RPM-GPG-KEY-qubes-r4.1-primary",
                    "qubes-templates-itl-testing": "/etc/qubes/repo-templates/keys/RPM-GPG-KEY-qubes-r4.1-primary-testing",
                },
            )
            self.assertAllCalled()
    ...
    def test_220_downgrade_skip_lower(
        ...
        with mock.patch(
            ...
            self.assertIn(
                "lower version already installed", mock_err.getvalue()
            )
        # Attempt to get download list
        self.assertEqual(
            mock_dl_list.mock_calls,
    ...
    def test_221_upgrade_skip_higher(
        ...
        with mock.patch(
            ...
            self.assertIn(
                "higher version already installed", mock_err.getvalue()
            )
        # Attempt to get download list
        self.assertEqual(
            mock_dl_list.mock_calls,
    ...
    def test_231_filter_version_reinstall(self):
        ...
        for key, val in [
            ...
            self.app.expected_calls[
                ("fedora-31", "admin.vm.feature.Get", f"template-{key}", None)
            ] = (b"0\0" + val.encode())
        for key, val in [
            ("name", "fedora-32"),
            ("epoch", "0"),
            ...
            self.app.expected_calls[
                ("fedora-32", "admin.vm.feature.Get", f"template-{key}", None)
            ] = (b"0\0" + val.encode())
        results = qubesadmin.tools.qvm_template.filter_version(
            query_res,
            self.app,
...
"""
        with mock.patch(
            "builtins.open", mock.mock_open(read_data=marker_vm)
        ) as mock_open:
            ...
            mock_open.assert_called_with(
                "/etc/os-release", "r", encoding="ascii"
            )
        self.assertAllCalled()
...
"""
        with mock.patch(
            "builtins.open", mock.mock_open(read_data=os_rel)
        ) as mock_open:
            ret = qubesadmin.tools.qvm_template.qubes_release()
            self.assertEqual(ret, "4.2")
            self.assertEqual(
...
"""
        with mock.patch(
            ...
            mock_open.assert_called_with(
                "/usr/share/qubes/marker-vm", "r", encoding="ascii"
            )
        self.assertAllCalled()
        ...
        def execute(pubkey, packagename):
            with open(packagename, "rb") as s:
                b = self.app.expected_service_calls[
                    ("test-vm", "qubes.TemplateDownload")
                ] = s.read()
            with tempfile.NamedTemporaryFile() as fd:
                ...
                qubesadmin.tools.qvm_template.verify_rpm(
                    fd.name, pubkey, template_name="invalid"
                )
            with self.assertRaises(
                qubesadmin.tools.qvm_template.SignatureVerificationError
            ):
                qubesadmin.tools.qvm_template.verify_rpm(
                    "/dev/null", pubkey, template_name="invalid"
                )
...
"""
    ...
    @mock.patch("subprocess.check_call")
    @mock.patch("rpm.TransactionSet")
    def test_400_migrate_from_rpmdb(
        self, mock_rpm_ts, mock_check_call, mock_getuid
    ):
        ...
        self.app.expected_calls[
            ("test-migrated", "admin.vm.feature.Get", "template-name", None)
        ] = b"0\0test-migrated"
        self.app.expected_calls[
            ("test-existing", "admin.vm.feature.Get", "template-name", None)
        ] = b"2\0QubesFeatureNotFoundError\0\0No such feature\0"
