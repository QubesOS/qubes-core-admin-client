...
class TC_00_RuleAction(qubesadmin.tests.QubesTestCase):
    ...
    def test_007_none_errors(self):
        ...
        with self.assertRaises(argparse.ArgumentError):
            self.action(
                None, ns, ["dsthost=127.0.0.1", "dstports=", "action=accept"]
            )
        with self.assertRaises(argparse.ArgumentError):
            self.action(
                None, ns, ["dsthost=127.0.0.1", "icmptype=", "action=accept"]
            )
...
class TC_10_qvm_firewall(qubesadmin.tests.QubesTestCase):
    ...
    def test_030_reset(self):
        ...
        self.app.expected_calls[
            ("test-vm", "admin.vm.firewall.Set", None, b"action=accept\n")
        ] = b"0\0"
        qubesadmin.tools.qvm_firewall.main(["test-vm", "reset"], app=self.app)
