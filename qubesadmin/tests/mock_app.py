...
class VolumeInfo:
    ...
    def __str__(self):
        ...
        if self.volume_type == "kernel":
            ...
            result += (
                f"path=/dev/mapper/" f"qubes_dom0-vm--{name}--{self.volume_type}\n"
            )
...
class MockQube:
    ...
    def update_calls(self):
        ...
        self.qapp.expected_calls[(self.name, "admin.vm.CurrentState", None, None)] = (
            response
        )
        ...
        for prop, value in self.properties.items():
            if (
                (prop == "template") and self.klass in ("TemplateVM", "StandaloneVM")
            ) or ((prop == "appvm_default_bootmode") and self.klass in ("AppVM",)):
        ...
        self.qapp.expected_calls[(self.name, "admin.vm.feature.List", None, None)] = (
        ...
        for tag in POSSIBLE_TAGS:
            if self.tags and tag in self.tags:
                continue
            self.qapp.expected_calls[(self.name, "admin.vm.tag.Get", tag, None)] = (
                b"0\x000"
            )

        if self.tags:
            self.qapp.expected_calls[(self.name, "admin.vm.tag.List", None, None)] = (
    ...
    def setup_volume_calls(self):
        self.qapp.expected_calls[(self.name, "admin.vm.volume.List", None, None)] = (
            b"0\x00root\nprivate\nvolatile\nkernel\n"
        )
        self.qapp.expected_calls[(self.name, "admin.vm.volume.Info", "root", None)] = (
            VolumeInfo(
                self.name,
                "root",
        ...
        ] = VolumeInfo(self.name, "private", usage_perc=self.usage).response_string()
        self.qapp.expected_calls[
            (self.name, "admin.vm.volume.Info", "volatile", None)
        ] = VolumeInfo(self.name, "volatile", usage_perc=self.usage).response_string()
        self.qapp.expected_calls[
    ...
    def setup_firewall_rules(self, rule_list: List[Dict[str, str]]):
        ...
        self.qapp.expected_calls[(self.name, "admin.vm.firewall.Get", None, None)] = (
            rules
        )
...
class MockDevice:
    ...
    def update_calls(self):
        ...
        current_response = self.qapp.expected_calls[
            (self.backend_vm, f"admin.vm.device.{self.dev_class}.Available", None, None)
        ]
        ...
        if self.assigned:
            for vm, mode, opts in self.assigned:
                current_response = self.qapp.expected_calls[
                    (vm, f"admin.vm.device.{self.dev_class}.Assigned", None, None)
                ]
                self.qapp.expected_calls[
                    (vm, f"admin.vm.device.{self.dev_class}.Assigned", None, None)
                ] = (current_response + self.assignment_string(vm, mode, opts).encode())
...
class QubesTestWrapper(QubesTest):
    def __init__(self):
        ...
        for pool in pools:
            self.app.expected_calls[("dom0", "admin.pool.UsageDetails", pool, None)] = (
        ...
        for qube in self._qubes.values():
            for feature in known_features:
                calls = [
                    ...
                    (qube.name, "admin.vm.feature.CheckWithTemplate", feature, None),
                ]
                for call in calls:
    ...
    def update_global_properties(self):
        ...
        self.app.expected_calls[("dom0", "admin.property.GetAll", None, None)] = (
            properties_getall
        )
...
class MockQubesComplete(MockQubes):
    ...
    def __init__(self):
        ...
        self._qubes["sys-usb"] = MockQube(
            ...
            qapp=self,
            running=True,
            features={"supported-service.qubes-u2f-proxy": "1", "servicevm": "1"},
        )
        ...
        self._qubes["test-red"] = MockQube(name="test-red", qapp=self, label="red")
        ...
        self._qubes["test-standalone"] = MockQube(
            name="test-standalone", qapp=self, klass="StandaloneVM", label="green"
        )
